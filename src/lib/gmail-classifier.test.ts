...
// Mock callClaude so tests don't spawn subprocesses
const mockCallClaude = mock(async (opts: any) => ({ text: '["receipt"]', isError: false }));

mock.module("./claude", () => ({
  callClaude: mockCallClaude,
}));

const { classifyEmail, isNccSender, extractReceiptDetails, extractSermonDetails } = await import("./gmail-classifier");

describe("classifyEmail", () => {
  it("returns classification from response", async () => {
    mockCallClaude.mockResolvedValueOnce({ text: '["receipt"]', isError: false });
  ...
  it("returns ignore for unrecognised response", async () => {
    mockCallClaude.mockResolvedValueOnce({ text: '["unknown_garbage"]', isError: false });
    const result = await classifyEmail({
...
describe("isNccSender", () => {
  it("matches New Creation Church domain", () => {
...
describe("extractReceiptDetails", () => {
  it("parses valid JSON from model response", async () => {
    mockCallClaude.mockResolvedValueOnce({
      text: JSON.stringify({ vendor: "Grab", amount: 12.5, currency: "SGD", date: "2026-03-24" }),
      isError: false,
    });
  ...
  it("returns fallback on JSON parse failure", async () => {
    mockCallClaude.mockResolvedValueOnce({ text: "not valid json at all", isError: false });
